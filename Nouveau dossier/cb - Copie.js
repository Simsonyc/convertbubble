  ...
  function openOverlay(config, wrapper) {
    ...
    if (config.branding && config.branding.enabled) {
      const header = el("div", {
        ...
        border:"2px solid #2563eb", marginBottom:"8px"
      });
      const label = el("div", { color:"#fff", fontWeight:"bold", fontSize:"14px" });
    ...
    video.src = vcfg.src || ""; video.controls = true; video.autoplay = true;
    video.playsInline = true; video.preload = "auto";
    videoWrap.appendChild(video);

    // CTA corrigé
    const ctasWrap = el("div", {
      position:"absolute", bottom:`${config.ctaOverlay?.offset ?? 80}px`, // plus haut pour laisser la barre native
    ...
    function makeCta(c) {
      const a = el("a", {
        flex:"1", textAlign:"center", // flex:1 => prend toute la largeur dispo
        background: config.theme?.primary || "#ff0055",
        padding:"10px 16px", borderRadius:"6px",
        color:"white", fontWeight:"bold", textDecoration:"none", display:"none"
      });
